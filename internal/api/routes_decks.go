...
func (s *Server) handleDrawCard(w http.ResponseWriter, r *http.Request) {
	...
	s.bus.Publish(Event{Type: EventCardDrawn, Payload: &CardDrawnPayload{CampaignID: RealtimeInt64(deck.CampaignID), SessionID: RealtimeInt64(body.SessionID), DeckID: RealtimeInt64(id), DeckName: RealtimePtr(deck.Name), Card: MustRealtimeObject(card), DrawIndex: RealtimeInt64(deck.DrawIndex + 1), Total: RealtimeInt64(len(order))}})

...
func TestIntegrityMigrationRewritesLegacyMapURLsWithinCampaignOnly(t *testing.T) {
	...
	punctuationMessageID := mustInsertID(t, seed, `INSERT INTO messages(session_id, role, content) VALUES (?, 'assistant', '[map](/api/files/maps/first.svg), sentence /api/files/maps/first.svg. quoted "/api/files/maps/first.svg" apostrophe ''/api/files/maps/first.svg''')`, sessionA)
	compactMarkdownMessageID := mustInsertID(t, seed, `INSERT INTO messages(session_id, role, content) VALUES (?, 'assistant', '[A](/api/files/maps/first.svg),[B](/api/files/maps/second.svg)')`, sessionA)
	commaSeparatedMessageID := mustInsertID(t, seed, `INSERT INTO messages(session_id, role, content) VALUES (?, 'assistant', '/api/files/maps/first.svg,/api/files/maps/second.svg')`, sessionA)
	...
	mustInsertID(t, seed, `INSERT INTO messages(session_id, role, content) VALUES (?, 'assistant', 'D /api/files/maps/overlap.svg /api/files/maps/overlap.svg.')`, sessionD)
	require.NoError(t, seed.Close())

	d, err := Open(path)
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, d.Close()) })
	var gotA, gotB, gotC, gotD, gotPunctuation, gotCompactMarkdown, gotCommaSeparated string
	...
	require.NoError(t, d.db.QueryRow(`SELECT content FROM messages WHERE id = ?`, punctuationMessageID).Scan(&gotPunctuation))
	require.NoError(t, d.db.QueryRow(`SELECT content FROM messages WHERE id = ?`, compactMarkdownMessageID).Scan(&gotCompactMarkdown))
	require.NoError(t, d.db.QueryRow(`SELECT content FROM messages WHERE id = ?`, commaSeparatedMessageID).Scan(&gotCommaSeparated))
	...
	assert.Equal(t, "D /api/files/maps/overlap.svg /api/files/maps/overlap.svg.", gotD)
	assert.Equal(t, fmt.Sprintf("[map](/api/assets/maps/%d), sentence /api/assets/maps/%d. quoted \"/api/assets/maps/%d\" apostrophe '/api/assets/maps/%d'", firstA, firstA, firstA, firstA), gotPunctuation)
	assert.Equal(t, fmt.Sprintf("[A](/api/assets/maps/%d),[B](/api/assets/maps/%d)", firstA, secondA), gotCompactMarkdown)
	assert.Equal(t, fmt.Sprintf("/api/assets/maps/%d,/api/assets/maps/%d", firstA, secondA), gotCommaSeparated)
}

...
CLAN BANES:
- Banu Haqim: Bane of Judgment — must spend vitae equal to Bane Severity when they harm an innocent; aggravated damage if they cannot.
- Hecata: Bane of the Living Grave — cannot feed from the dead or those with less than half their blood remaining; must feed from the living only.
...
- Ravnos: Bane of Rötschreck — sunlight triggers Rötschreck automatically at Bane Severity difficulty; fire is +Bane Severity difficulty to resist; Ravnos burn faster than other Kindred.
- Salubri: Bane of the Third Eye — a third eye opens on their brow whenever they use Disciplines; Auspex use is visible to anyone with Auspex 1+; mortals who see it must succeed at a Composure + Occult roll or flee.
- Tzimisce: Bane of Old Soil — must sleep with at least 2 lbs of earth from their homeland or haven; failing to do so means rising each night with all Hunger dice showing as Hunger dice (maximum Hunger for the first action).
...
- Ghoul: Mortal servant granted vitae by a domitor. No Hunger track. Has one Discipline at 1 dot (their domitor''s gift). Bond Strength 1-3 (blood bond; 3 is fully bound). Ghouls age normally without monthly vitae; withdrawal causes painful degeneration. The domitor''s clan resonance affects the ghoul''s emotions.
